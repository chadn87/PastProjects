...
//Functions to use Ajax to get earthquake data
/////////////////////////////////////////////////////////////////////////////
/*
//this version of the function uses the GeoJSON urls
function whichEarthquakeData(){

  if (document.getElementById('hourQuake').checked){
  //past hour all quakes uses GeoJSON
  var url = 'http://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_hour.geojson';
  getEarthquakes(url);
  }
  if (document.getElementById('dayQuake').checked){
  //past day all quakes uses GeoJSON
  var url = 'http://earthquake.usgs.gov/earthquakes/feed/v0.1/summary/all_day.geojson';
  getEarthquakes(url);
  }
  if (document.getElementById('weekQuake').checked){
  //Past 7 days uses GeoJSON
  var url = 'http://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson';
  getEarthquakes(url);
  }
  if (document.getElementById('monthQuake').checked){
  //Past 30 days uses GeoJSON
...
function getEarthquakes(url){
  ...
  //usgs.gov is in the process to stop using GeoJSONP, but they currently have Cross Origin Resource Sharing
  //undefined so therefore we cannot pull the data using this method until they define it.
  //to use this method comment lines 161-211 and uncomment 242-272 
  //if using google chrome press f12 then go to console to see the XMLHttprequest error showing that the CORS is not set
var quakeInfo = new XMLHttpRequest();
quakeInfo.open('GET',url, true);
quakeInfo.onreadystatechange = function() {
	//if statement to ensure the url is valid
	if(quakeInfo.readyState == 4 && quakeInfo.status == 200){
	 earthQuakeData(quakeInfo.results);
	}
  };
  quakeInfo.send();
}

  function earthQuakeData(results) {
  var Smag = "Magnitude: "
  for (var i = 0; i < results.features.length; i++) {
    var earthquake = results.features[i];
	var magnitude = earthquake.properties.mag;
    ...
    var quakeMarker = new google.maps.Marker({
      position: quakeLatLng,
      map: map,
	  title: Smag.concat(String(magnitude))
    });
   }
  }
*/
